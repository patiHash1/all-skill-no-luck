...
export default function Leaderboard() {
  ...
  const getWeeklyScore = (user, week) => {
    ...
    const base = mode === 'overall' ? user.score : Math.floor((user.score / 4) + (seed % 5));
    return user.isYou ? (mode === 'overall' ? userScore : Math.floor(userScore / 4)) : base;
  };
          ...
          {[1, 2, 3, 4].map(w => (
            <button
              ...
              style={{
                padding: '0.4rem 1rem',
                borderRadius: '20px',
                border: activeWeek === w ? '1px solid var(--color-gold)' : '1px solid rgba(255,255,255,0.1)',
                background: activeWeek === w ? 'rgba(212, 175, 55, 0.1)' : 'transparent',
                color: activeWeek === w ? 'var(--color-gold)' : 'var(--color-muted)',
                fontSize: '0.8rem',
                cursor: 'pointer',
                whiteSpace: 'nowrap'
              }}
                    ...
                    <td>
                      <div className="score-bar-track">
                        <div
                          ...
                          style={{ width: `${(user.displayScore / (mode === 'overall' ? MAX_SCORE : 18)) * 100}%` }}
                        />
                      </div>
                      <span style={{ fontSize: '0.7rem', color: 'var(--color-muted)' }}>
                        {user.displayScore} / {mode === 'overall' ? MAX_SCORE : 18} {t('leaderboard.pts')}
                      </span>
                    </td>
                  </tr>
                ))}
