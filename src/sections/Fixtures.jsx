...
function FixtureCard({ match, t, onTeamClick }) {
  ...
  return (
    <div className="match-card">
      {/* Match Info Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: 'var(--color-muted-2)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        <span>🗓 {match.date} • {match.time}</span>
        <span>🏟 {match.stadium}</span>
      </div>

      <div className="match-teams" style={{ paddingBottom: '0.5rem' }}>
        <div className="team-col" onClick={() => onTeamClick(homeName, match.home)} title={`View ${homeName} Stats`} style={{ cursor: 'pointer' }}>
          <span className="team-flag"><FlagIcon team={match.home} /></span>
          <span className="team-name">{homeName}</span>
        </div>
        <span className="vs-label">{t('predict.vs')}</span>
        <div className="team-col" onClick={() => onTeamClick(awayName, match.away)} title={`View ${awayName} Stats`} style={{ cursor: 'pointer' }}>
...
function GroupStandings({ group, t, onTeamClick }) {
  ...
  return (
    <div className="standings-card" style={{ marginTop: '2rem' }}>
      <h3 className="section-subtitle" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', background: 'rgba(255, 255, 255, 0.03)' }}>
        <table className="leaderboard-table" style={{ margin: 0 }}>
          ...
          <tbody>
            {teams.map((team, idx) => (
              <tr key={team} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
                <td style={{ textAlign: 'center', color: 'var(--color-muted-2)', fontSize: '0.8rem' }}>{idx + 1}</td>
                <td 
                  onClick={() => onTeamClick(t(`teams.${team}`, team), team)} 
                  style={{ cursor: 'pointer', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                >
                ...
                <td style={{ textAlign: 'center', fontSize: '0.85rem' }}>0</td>
                <td style={{ textAlign: 'center', fontSize: '0.85rem' }}>0</td>
                <td style={{ textAlign: 'center', fontWeight: 'bold', color: 'var(--color-gold)' }}>0</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <h1 className="page-title">{t('fixtures.title', 'Fixtures & Stats')}</h1>
        </div>
      </div>
      <p className="page-sub">
        {t('fixtures.sub', 'Select a group to view all matches and team rosters')}
      </p>
