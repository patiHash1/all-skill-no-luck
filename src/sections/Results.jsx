...
export default function Results() {
  const { predictions, setPrediction, predictionCount, clearPredictions } =
    usePredictions();
  ...
  const CHIP_LABEL = {
    home: (m) => (
      ...
      >
        <FlagIcon team={m.home} /> {t(`teams.${m.home}`, m.home).split(" ")[0]}{" "}
        {t("results.win")}
      </span>
    ),
    draw: () => t("results.draw"),
    away: (m) => (
      <span
        style={{ display: "inline-flex", alignItems: "center", gap: "4px" }}
      >
        <FlagIcon team={m.away} /> {t(`teams.${m.away}`, m.away).split(" ")[0]}{" "}
        {t("results.win")}
      </span>
    ),
  };
  ...
  return (
    <div className="page">
        ...
        <div className="match-grid">
          {upcomingMatches.map((match) => {
            ...
            return (
              <div
                key={match.id}
                className={`match-card ${locked ? "locked" : ""}`}
              >
                <div
                  style={{
                    display: "flex",
      ...
      <div className="results-summary">
        ...
        <div className="summary-card">
          <span className="val">
            {Math.round((predictionCount / total) * 100)}%
          </span>
      ...
      <div style={{ marginBottom: "2rem" }}>
        <div className="progress-track">
          <div
            ...
            style={{ width: `${(predictionCount / total) * 100}%` }}
          />
        </div>
        <span style={{ fontSize: "0.75rem", color: "var(--color-muted)" }}>
          {t("results.progress_text1")}
          {predictionCount}
          {t("results.progress_text2")}
          {total}
          {t("results.progress_text3")}
        </span>
      </div>
            ...
            {picked.map((match) => {
              ...
              return (
                <div key={match.id} className="prophecy-row">
                  ...
                  >
                    <FlagIcon team={match.home} />{" "}
                    {t(`teams.${match.home}`, match.home)}{" "}
                    <span style={{ margin: "0 0.25rem" }}>
                      {t("results.vs")}
                    </span>{" "}
                    <FlagIcon team={match.away} />{" "}
                    {t(`teams.${match.away}`, match.away)}
                  </span>
